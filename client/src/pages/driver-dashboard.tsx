...
export default function DriverDashboard() {
  ...
  const getVehicleColor = () => {
    const color = vehicleColors.find(c => c.name === driver?.vehicleColor);
    return color || { value: "#9ca3af", textColor: "white", name: "Grey" };
  };

  const VehicleIcon = driver?.vehicleType === "bike" ? Bike : Car;
        ...
        <div className="container mx-auto flex h-16 items-center justify-between gap-4 px-4">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-medium" data-testid="text-driver-name">{driver?.fullName || "Driver"}</p>
              ...
              <CardContent>
                <div className="flex flex-col items-center">
                  ...
                  <div className="text-center">
                    <p className="font-medium capitalize">{driver?.vehicleType || "Keke"}</p>
                    <p className="text-sm text-muted-foreground">{getVehicleColor().name}</p>
                    <p className="text-lg font-bold mt-2">{driver?.vehicleNumber || "GE 4231-24"}</p>
                  </div>
                </div>
                <div className="mt-4 pt-4 border-t border-ash dark:border-border">
                  <div className="flex items-center gap-2 text-sm">
                    <Phone className="h-4 w-4 text-muted-foreground" />
                    <span>{driver?.phoneNumber || "0593528296"}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
